...
class Cloud {
    ...
    //加事件
    Event(){  
        this.flle = document.getElementsByClassName('file-item');
        this.fileItem = Array.from(this.flle);
        ...
        this.fileItem.forEach((e,i) =>{
            this.i = e.children[3];
            this.img = e.children[0];
            this.i.addEventListener('click',this.Click.bind(this,this.i));
            this.img.addEventListener('dblclick',this.Dblclick.bind(this,i));
            this.i.onOff = true;  
        });
    ...
    Click(e){
        ...
        if(e.onOff){
            ...
            e.parentNode.className = 'file-item active';
            e.className = 'checked';
            e.onOff = !e.onOff;

        }else{
            e.parentNode.className = 'file-item'
            e.className = '';
            e.onOff = !e.onOff;   
        }
        if(e.className != ''){
            this.rename.addEventListener('click',this.Rename.bind(this,e.parentNode));
            document.addEventListener('keydown',this.Keydown.bind(this,e.parentNode));
            this.del.addEventListener('click',this.Del.bind(this,e.parentNode));
            //e.dataset.index行间属性
        } 
        ...
        collision(){  //对勾
            this.fileItem.forEach((e,i)=>{
                ...
                if(left > Yright || top > Ybottom || right < Yleft || bottom < Ytop){
                    ...
                    e.children[3].onOff = false;
                }
                this.folders.addEventListener('click',(ev)=>{
                    if(!this.targetParent(ev.target)){
                        e.children[3].className = '';
                        e.className = 'file-item';
                        e.children[3].onOff = !e.children[3].onOff;
                    }
                });
            });    
        }
        //看他点击的是不是点击的盒子  如果不是盒子   就触点击大盒子父级的点击事件
        targetParent(ele){
